...
begin
  ...
  task :heckle => :verify_rcov do
    ...
    ObjectSpace.each_object(Module) do |mod|
      ...
      # only process objects in the root module namespace
      ...
      spec_methods.each do |method|
        ...
        unless spec_file.file?
          puts "No spec file #{spec_file} for #{mod}##{method}"
        end
      ...
      other_methods.each do |method|
        ...
        ObjectSpace.each_object(Module) do |descendant|
          next unless descendant.name =~ /\A#{root_module}(?::|\z)/ && mod >= descendant
          descendant_spec_prefix = spec_dir.join(descendant.name.underscore)
          descendant_specs.concat(Pathname.glob(descendant_spec_prefix.join('*_spec.rb')))

          if method.to_s == 'initialize'
            descendant_specs.concat(Pathname.glob(descendant_spec_prefix.join('class_methods/new_spec.rb')))
          end
        end

        specs << [ "##{method}", descendant_specs ]
      end
      ...
      other_class_methods.each do |method|
        ...
        ObjectSpace.each_object(Module) do |descendant|
          next unless descendant.name =~ /\A#{root_module}(?::|\z)/ && mod >= descendant
          descendant_spec_prefix = spec_dir.join(descendant.name.underscore)
          descendant_specs.concat(Pathname.glob(descendant_spec_prefix.join('class_methods/*_spec.rb')))
        end
